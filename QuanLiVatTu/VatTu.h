...
AVLTree rightRotate(AVLTree& root) {//Xoay phai
...
AVLTree leftRotate(AVLTree& root) {//Xoay trai
...
//===============Them Vat Tu================
AVLTree addVatTu(AVLTree& root, VatTu& vattu) {//Sap xep vat tu theo maVT
    if (root == NULL) {
...
int CountVatTu(AVLTree& root) {//Dem so luong vat tu
    if (root == NULL) {
        return 0;
    }
...
//===============Tim Vat Tu================
AVLTree findVatTu(AVLTree& root, char* TimMaVT) { //Tim vat tu theo ma vat tu
    if (root != NULL) {
...
VatTu** GetTempArray(AVLTree& root, VatTu* vt[], int& n) {//Lay tat ca vat tu trong cay (khoi tao n=0,vt[] cap phat dong)
    if (root != NULL) {
        if (root->data.soLuongTon != 0) {
            vt[n] = &root->data;
            n++;
        }
        GetTempArray(root->left, vt, n);
        GetTempArray(root->right, vt, n);
    }
    return vt;
}
VatTu** SX_TenVT(VatTu* vt[], int n) {//sap xep vat tu theo ten
    VatTu* x;
    for (int i = 0; i < n; i++) {
        for (int j = i + 1; j < n; j++) {
...
void removeAll(AVLTree& root) {//Xoa toan bo dsVT
    if (root != NULL) {
        removeAll(root->left);
...
void DocFileVatTu(AVLTree& root) {
    ...
    if (f.fail()) {
        ...
        getline(f, data);
        strcpy(vt.tenVT, data.c_str());
        getline(f, data);
        strcpy(vt.DVT, data.c_str());
        getline(f, data);
