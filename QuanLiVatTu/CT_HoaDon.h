...
PTRCT_HoaDon newNode(CT_HoaDon x){ // tao 1 node moi
...
int TongTienMotVatTu(PTRCT_HoaDon node){
    int sum = 0;
    PTRCT_HoaDon p = node;
    sum += p->info.SoLuong * p->info.DonGia;
    return sum;
}
long TongTienHoaDon(PTRCT_HoaDon first){
    long Total = 0;
    PTRCT_HoaDon p = first;
    while(p != NULL){
        Total += TongTienMotVatTu(p);
        p = p->next;
    }
    return Total*(1+p->info.VAT);
}
